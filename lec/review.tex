...
\begin{figure}
  \begin{tikzpicture}
    \begin{semilogyaxis}[
      ...
      \addplot table[x=k,y=N] {rev_itercomp.dat};
      \addplot table[x=k,y=G] {rev_itercomp.dat};
      \addplot table[x=k,y=B] {rev_itercomp.dat};      
    \end{semilogyaxis}
  \end{tikzpicture}
...
We now consider the problem of numerically finding solutions $x^* \in
\bbR^n$ to
\[
  f(x^*) = 0,
\]
where $f : \bbR^n \rightarrow \bbR^n$.  We will assume throughout that
$f$ is continuously differentiable.  We have already addressed the
case where $f$ is an affine function (i.e. a constant plus a linear part):
\[
  f(x) = Ax-b,
\]
and many of the ideas we explored in the linear setting carry over to
the nonlinear case.  However, the nonlinear case can get much more
complicated, for two main reasons:
\begin{itemize}
\item
  The equation can have no solutions or many solutions.  If there are
  many solutions, it may be that only a few (or only one) are actually
  interesting in the context of an application.
\item
  Even if a solution exists, we typically have no efficient way to
  find it without additional information about the structure of the
  equations or an initial guess.
\end{itemize}
Of course, the difficulty of the problem in general is no reason to
throw up our hands when it comes to particular instances of the
problem.  It just means that we need to think!

\section*{Affine approximation}

An {\em affine} function $g : \bbR^n \rightarrow \bbR^m$
is a function of the form
\[
  g(x) = b + Ax
\]
where $A \in \bbR^{n \times m}$ is a matrix (representing a linear
function) and $b \in \bbR^n$ is a fixed vector.  We 
spent most of the semester thinking about computing with affine
functions.  For example, if $g(x) = b + Ax$ and $f(x) = d + Cx$
are affine functions (and dimensions make sense), composition just
involves matrix multiplication:
\[
  f(g(x)) = d + C(b + Ax) = (d + Cb) + CA x = f(g(0)) + CAx.
\]
We also know how to invert $g$, assuming
$g : \bbR^n \rightarrow \bbR^n$ is invertible:
\[
  g^{-1}(y) = A^{-1}(y-b)
\]
Similarly, it's easy to write down what happens if we add, subtract,
or scale affine functions.

The main calculus fact that we need is that nice (differentiable)
functions are locally well approximated by affine functions.  That is,
we have
\[
  g(x+z) = g(x) + J(x) z + e(z)
\]
where $J(x) = g'(x)$ is the {\em Jacobian} matrix at $x$ with entries
\[
  J_{ik} = \frac{\partial g_i}{\partial x_k}
\]
and the error function $e(z)$ is $O(\|z\|^2)$ if $g$ is twice
differentiable at $x$.  For example, the fact that we know how to
compose affine functions gives us the chain rule; we just compose
affine approximations to $f$ and $g$
\[
  f(g(x+z)) = f(g(x)) + J_f(g(x)) \, J_g(x) \, z + O(\|z\|^2).
\]
And if $f : \bbR^{n} \rightarrow \bbR^{n}$ has a nonsingular Jacobian
at $x$, it has a locally-defined inverse $f^{-1}$ with
\[
  f^{-1}\left( f(x)+y \right) = J(x)^{-1} ( y-f(x) ) + O(\|y\|^2).
\]
One does have to pay attention to the fact that these approximations are

\subsection*{Think one direction at a time}
\subsection*{From multi-dimensional to one-dimensional}
\subsection*{Derivatives and gradients}
\subsection*{The Jacobian matrix and first-order Taylor}
\subsection*{The Hessian matrix and second-order Taylor}

\section*{Fixed point iteration revisted}

\subsection*{Examples}

\subsection*{Gauging progress}

\subsection*{The convergence recipe}

\section*{Model-based methods}

\section*{Newton for systems}

\subsection*{Quadratic convergence}

\subsection*{Diagnosing Newton}

\section*{Issues with Newton}

\subsection*{Difficult derivatives}

\subsection*{Expensive linear algebra}

\subsection*{Local convergence}

\section*{Unconstrained optimization}

\section*{Newton for optimization}
